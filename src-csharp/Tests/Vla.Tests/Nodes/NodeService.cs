...
public class NodeService
{
	...
	public class NumberConstantNode : Node
	{
		...
		[NodeProperty] public NumberType Mode { get; set; } = NumberType.Integer;
		...
		{
			[NodeEnumValue("Integer")] Integer,
			[NodeEnumValue("Decimal")] Decimal
		}
	}
	...
	public void NodeService_GeneratesCorrectTypes()
	{
		...
		Assert.That(definition.PossibleValues.Select(x => x.Value), Is.EquivalentTo(new[] { NumberConstantNode.NumberType.Integer, NumberConstantNode.NumberType.Decimal }));
	}
	...
	public void NodeService_Structure_CorrectProperties()
	{
		...
		Assert.That(structure.Properties.Select(x => x.Type), Is.EquivalentTo(new[] { typeof(double), typeof(NumberConstantNode.NumberType) }));
