    ...
    private static Result<NodeStructure> BuildStructure(Type type)
    {
        var structure = new NodeStructure()
            ...
            .WithProperties(type.GetProperties()
                ...
                .Select(y =>
                    new PropertyStructure(y.GetCustomAttribute<NodePropertyAttribute>()?.Name ?? y.Name, y.PropertyType, JsonConvert.SerializeObject(y.GetValue(Activator.CreateInstance(type)))))
                .ToArray())
